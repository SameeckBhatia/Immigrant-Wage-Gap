...
```{r}
# Uncomment the below line and run the chunk to install packages
# install.packages(c("tidyverse", "stargazer", "knitr"))
```
...
cis_clean |>
  ...
  labs(x = "Wage ($/hour)", y = "Count")
```

Figure \@ref(fig:fig1) shows a heavy right skew in the distribution of hourly wages, with the most common values in the \$20--40/hour wage range. There are around 1,000 observations in the \$0--10/hour range, which is small but worth considering. Frequency decreases as hourly wages increase, and the distribution's shape makes estimating effects for high-wage earners more challenging.
...
stargazer(model1, model2, model3, coef = list(coef1, coef2, coef3),
          omit = "province", dep.var.labels = c("ln(Hourly Wage)"),
          intercept.bottom = FALSE, header = FALSE, 
          keep.stat = c("rsq", "ser"), font.size = "small",
          add.lines = list(c("Province Controls", "Yes", "Yes", "Yes")),
          digits = 3, digit.separator = ",", title = "Model Summary",
          label = "tab:tab2")
```

```{r fig2, fig.width=4, fig.height=2.5, fig.align='center', fig.cap='Hourly Wages by Immigration Length'}
# Plotting bivariate data
cis_clean |>
  ggplot(aes(x = as.factor(imm_length_level), y = hourly_wage)) +
  geom_boxplot(fill = "tan", color = "black", width = 0.5, outliers = FALSE) + 
  labs(x = "Immigration Length", y = "Wage ($/hour)")
```


Table \@ref(tab:tab2) presents the summary of three regression models, with the intercept and coefficients presented on the natural scale to reflect baseline hourly wages and percentage changes, respectively. Model 1 includes all control variables except for highest education level and immigration length level. Model 2 adds the highest education level control but omits immigration length level. Model 3 includes all variables and controls as outlined in Section 4.

Figure \@ref(fig:fig2) shows the raw wage gap by immigration length. Initially, there is an observable wage difference between immigrants who have been in Canada for less than a decade and citizens. Wages tend to level off and even surpass those of citizens as immigrants become more integrated. Although the upward trend is in line with the theory and expectations, this would imply the raw gap is nearly zero, or slightly in favor of immigrants. However, this is not the case as Table \@ref(tab:tab2) shows that the raw gap does not account for other control variables that must be considered.
